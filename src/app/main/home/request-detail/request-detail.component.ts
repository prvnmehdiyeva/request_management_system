...
export class RequestDetailComponent implements OnInit {
  ...
  nowDate() {
    const now = new Date();
    this.now = now.toLocaleString('en-US', {
      weekday: 'long',
      month: 'short',
      day: 'numeric',
      hour: 'numeric',
  ...
  addComment() {
    ...
    this.commentsService
      ...
      .pipe(take(1))
      .subscribe((_) => this.getCommentSubject.next(true));
  ...
  addInfo() {
    ...
    if (
      controlsToCheck.some(
        (controlName) => this.formRequest.get(controlName)?.invalid
      )
    ) {
    ...
    const formRequestData = {
      ...
      type:
        document
          .querySelector('input[name="inlineRadioApp"]:checked')
          ?.getAttribute('value') || '',
      sender: this.formRequest.get('sender')?.value || '',
      solman: this.formRequest.get('solman')?.value || '',
      contact:
        document
          .querySelector('input[name="inlineRadio"]:checked')
          ?.getAttribute('value') || '',
      code: this.formRequest.get('code')?.value || '',
      root: this.formRequest.get('root')?.value || '',
      routine:
        (document.getElementById('gridCheck') as HTMLInputElement)?.checked ||
        false,
    };
    ...
    this.request$ = this.request$.pipe(
      map((requests) => {
        return requests.map((request) => {
          if (request.id.toString() === this.requestId) {
            this.requestService
              .updateRequestStatus(this.requestId, formRequestData, request)
              .subscribe(() => {
                this.showToast();
